...
\item Brezger, A., Steiner, W. (2008): Monotonic regression based on Bayesian P-splines: an application to estimating price response functions from
store-level scanner data. {\it Journal of Economic and Business Statistics}, {\bf 26}, 90--104. \vspace{-0.25cm}
...
\item Klein, N., Kneib, T., Lang, S., Sohn, A. (2014): Bayesian structured additive distributional regression.  Under revision for {\it Annals of Applied Statistics}.
\item Klein, N., Kneib, T., Lang, S. (2014): Bayesian Generalized Additive Models for Location, Scale and Shape for Zero-Inflated and Overdispersed Count Data. To appear in {\it Journal of the American Statistical Association}.
\item Klein, N., Kneib, T., Klasen, S., Lang, S.(2014): Bayesian structured additive distributional regression for Multivariate Responses. To appear in {\it Journal of the Royal Statistical Society C}.
...
\item Lang, S., Brezger, A. (2004): Bayesian P-Splines {\it Journal of Computational and Graphical Statistics}, {\bf 13},
    183--212.
\item Lang, S., Umlauf, N., Wechselberger, P., Hartgen, K. and Kneib, T. (2014): Multilevel Structured Additive Regression, {\it Statistics and Computing}, {\bf 24}, 223--238
\end{itemize}
...
\[
\varthetavec_k = h_k(\etavec_k) = h_k(\Xvec_{1k} \betavec_{1k} + \ldots + \Xvec_{qk} \betavec_{qk} + \Uvec_k \gammavec_k), \quad k=1,\ldots,K.
\]
...
in $\boldsymbol{\beta}$ corresponds to the regression coefficient of the $k$-th cluster.
We now define a second level equation
\begin{equation}
\label{eq:compprior}
\boldsymbol{\beta}_j = \boldsymbol{\eta}_j + \boldsymbol{\varepsilon}_j =
\mathbf{X}_{j1} \boldsymbol{\beta}_{j1} + \ldots + \mathbf{X}_{jq_j} \boldsymbol{\beta}_{jq_j} +
\mathbf{U}_j \boldsymbol{\gamma}_j +  \boldsymbol{\varepsilon}_j,
\end{equation}
where the terms $\mathbf{X}_{j1} \boldsymbol{\beta}_{j1},\dots,\mathbf{X}_{jq_j} \boldsymbol{\beta}_{jq_j}$ correspond to additional nonlinear
functions $f_{j1},\dots,f_{jq_j}$ and
