...
\begin{equation}
\label{ZEqnNum341486}
p\left(\beta _{s} |\sigma^2,\tau _{s}^{2} \right)\propto \frac{1}{\det \left(K_{s} \right)^{{1
\mathord{\left/ {\vphantom {1 2}} \right. \kern-\nulldelimiterspace} 2} } } \exp
\left(-\frac{1}{2} \beta _{s} ^{{'} } K_{s}^{-1} \beta _{s} \right),
\end{equation}
...
which results in a marginal (respective $\tau_{j}^2$) Gaussian distribution of the regression coefficients $\beta _{s} |\sigma^2,\lambda \mathop{\sim
}\limits^{i.i.d.} N\left(0,{\lambda \mathord{\left/ { \vphantom {\sigma^2  2\lambda}} \right. \kern-\nulldelimiterspace} 2\sigma^2} \right).$
The logarithm of the common marginal prior for $\beta _{s} |\sigma^2,\lambda $ corresponds to the ridge penalty
...
distributions $\beta _{j} |\sigma^2,\lambda \mathop{\sim }\limits^{i.i.d.} Laplace\left(0,
...
\kern-\nulldelimiterspace} \tau _{j}^{2} } $ is replaced by the pnalty matrix of the shrinkage priors $K_{s}^{-1} $. For example for
...
^{{'} } CX_{s} +K_{s}^{-1} \right)^{-1} ,\quad m_{s} =P_{s}^{-1} \frac{1}{\sigma ^{2}
...
=b_{\sigma } +\frac{1}{2} \varepsilon '\varepsilon +\frac{1}{2} \beta '{(K_{s}/\sigma^2)}^{-1}
\beta \]

and the corresponding variances matrix $K_{s}/ \sigma^2 =diag\left(\tau _{p+1}^{2} ,\ldots,\tau _{p+q}^{2} \right)$ for the ridge, lasso and NMIG.
...
\section{Multilevel structured additive disributional regression}
...
\begin{align*}
\eta_i = f_1(x_{i1}) + \ldots + f_q(x_{ip}) + \uvec_i' \gammavec, \,\,\,\,\,\,\,\, i = 1, \ldots, n,
\end{align*}
by $\mu_i = h(\eta_i)$, see Chapter \ref{obsmodel}. In matrix notation we obtain 
$$
\etavec = \Xvec_1 \betavec_1 + \ldots + \Xvec_p \betavec_p + \Uvec \gammavec,
$$
for the predictor, see again Chapter \ref{obsmodel} for details.
...
A more flexible approach is given by distributional regression as introduced in  \citeasnoun{kleknelan14a} and \citeasnoun{kleknelan14b}. On the one hand, the class of distributions that can be estimated with distributional regression is not restricted to the exponential family. On the other hand, distributional regression allows to model not only the conditional mean of the response variable but the whole set of distribution parameters, i.e.~all parameters for the location, scale and shape of a distribution can be related to a set of predictor variables, which of course may vary between the different parameters.

For most distribution families a location parameter $\muvec$, a scale parameter $\sigmavec$ (or $\sigmavec^2$) and a maximum of two shape parameters $\nuvec$ and $\tauvec$ are sufficient to fully characterize the respective distribution. Using response functions $h_1, \ldots, h_4$ each of these parameters can be linked to a structured additive predictor
\begin{align}
\begin{array}{lllll}
\muvec & = & h_1(\etavec_1) & = &    h_1(\Xvec_{11} \betavec_{11} + \ldots + \Xvec_{q1} \betavec_{q1} + \Uvec_1 \gammavec_1), \\ [0.2cm]
\sigmavec & = & h_2(\etavec_2) & = & h_2(\Xvec_{12} \betavec_{12} + \ldots + \Xvec_{q2} \betavec_{q2} + \Uvec_2 \gammavec_2), \\ [0.2cm]
\nuvec & = & h_3(\etavec_3) & = &    h_3(\Xvec_{13} \betavec_{13} + \ldots + \Xvec_{q3} \betavec_{q3} + \Uvec_3 \gammavec_3), \\ [0.2cm]
\tauvec & = & h_4(\etavec_4) & = &   h_4(\Xvec_{14} \betavec_{14} + \ldots + \Xvec_{q4} \betavec_{q4} + \Uvec_4 \gammavec_4).
\end{array}
\label{eq:gamlss}
\end{align}
Usually, the response functions are chosen to ensure appropriate restrictions on the parameter spaces. We use, for example, the exponential function to ensure positivity of the scale parameter, i.e.~$\sigmavec = \exp(\etavec_2)$.
...
A comprehensive list of all distributional regression models available in BayesX can be found in ??
...
Bayesian quantile regression requires a distributional assumption for the responses to be able to set up a likelihood. Following \citeasnoun{WalKneLanYue12} we will assume independent and identically distributed observations following an asymmetric Laplace distribution with location parameter $\xvec_i' \betavec_{\varphi}$, scale parameter $\sigma^2$ and skewness parameter $\varphi$,
\begin{align*}
y_{i} | \betavec_{\varphi}, \sigma^2, \varphi \overset{\textnormal{iid}}{\sim} \textnormal{ALD}(\xvec_i' \betavec_{\varphi},\sigma^2,\varphi).
\end{align*}
Then, the density of the responses is given by
\begin{align*}
p(y_i | \betavec_{\varphi},\sigma^2,\varphi) = \frac{\varphi(1-\varphi)}{\sigma^2} \exp\left(-\frac{\rho_{\varphi}(y_i - \xvec_i' \betavec_{\varphi})}{\sigma^2}\right).
\end{align*}
Maximizing the corresponding posterior (for fixed $\sigma^2$ and $\varphi$)
\begin{align*}
p(\betavec_{\varphi} | \yvec, \sigma^2, \varphi) & \propto \prod_{i=1}^n p(y_i | \betavec_{\varphi}, \sigma^2) \\
& \propto \exp \left(-\frac{1}{\sigma^2}\sum_{i=1}^n \rho_{\varphi}(y_i - \xvec_i' \betavec_{\varphi})\right)
\end{align*}
with respect to $\betavec_{\varphi}$ obviously is equivalent to minimizing the AWE criterion (\ref{eq:quantmin}). However, in contrast to frequentist quantile regression the linear predictor $\eta_{i,\varphi} = \xvec_i' \betavec_{\varphi}$ can be replaced by a hierarchical structured additive predictor without any further difficulties, see \citeasnoun{WalKneLanYue12} for details.
...
\end{align*}
with $\xi = \frac{1-2\varphi}{\varphi(1-\varphi)}$ and $\delta^2 = \frac{2}{\varphi(1-\varphi)}$.  $W \sim \textnormal{Exp}(\frac{1}{\sigma^2})$ and $Z \sim \mathcal{N}(0,1)$ are independent random variables following an exponential distribution with mean $\sigma^2$ and a standard normal distribution, respectively. Thus, using offsets $\xi W$ and weights $\delta \sqrt{\sigma^2 W}$ the Bayesian quantile regression problem can be interpreted as a conditionally Gaussian regression model after imputing $W$ as a part of the MCMC sampler.
...
\harvarditem{Klein et al.}{2014}{KleDenKneLan14}
...
{\it To appear in Journal of the American Statistical Association}.


\harvarditem{Klein, Kneib \& Lang}{2014}{KleKneLan14c}
{\scshape Klein, N., Kneib, T. \& Lang, S.}(2014).
Bayesian structured additive distributional regression.
{\it Revised for Journal of the Royal Statistical Society C}.
