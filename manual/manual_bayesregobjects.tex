...
\citeasnoun{BreLan06}, \citeasnoun{FahOsu06} and \citeasnoun{HenBreFah06}. Introductions to
(Bayesian) generalized linear models as well as to  semi- and
nonparametric models are given in \citeasnoun{fahkne13}.
...
gamma, binomial, multinomial or Poisson. In addition,
...
specification of weights is not allowed. Similarly, for hazard
regression and multi-state models, weighted regression is not
implemented yet.
...
& 2. The options #a# and #b# specify the hyperparameters of the Gamma prior
  for the shrinkage parameter $\lambda$ if the lasso- or the ridge-prior is used (**).
& #a=0.001#, #b=0.001# \\

& 3. The options #a# and #b# specify the hyperparameters the inverse Gamma prior
  for the variance parameters $t_{j}^2$ if the NMIG-penalty is used.
& #a=5#, #b=50# \\
\hline

#adaptive#
& Specifies adaptive versions of the shrinkge priors.
& #-# \\
\hline

#aw#,#bw#
& The options #aw# and #bw# specify the hyperparameters of
  the Beta prior for the parameter $\omega$ if the NMIG-penalty is used (**).
& #a=0.001#, #b=0.001# \\
 \hline
...
#w#
& Specifies the starting value of the complexity parameter $\omega$ if the NMIG-penalty is used. Values have to
...
& Specifies if the complexity parameter $\omega$ should take the fixed value given
  in option #w# if the NMIG-penalty is used (**).
& - \\
\hline
...
& Specifies if the shrinkage parameter $\lambda$ should take the fixed value
  given in option #shrinkage# (*).
& - \\
\hline


#shrinkage#
& Provides a starting value of for the shrinkage parameter $\lambda$ if the lasso-
  or ridge -penalty is used. Values have to be positive (**).
& #shrinkage=1# \\
\hline

#t2#
& Provides a starting value of for the variance parameter $t_{j}^{2}$ of the corresponding
  effect if the NMIG-penalty is used. Values have to be positive and have to be set in each shrinkage term.
& #t2=11# \\
\hline

#tau2#
& Provides a starting value of for the variance parameter $\tau _{j}^{2} $
of the corresponding effect if the lasso- or the ridge-prior is used.
...
logit or probit link), Poisson and gamma. Supported
...
response, and Poisson models  with the log-link. For some distributions
...
responses
and $\theta = \mu$ for non-Gaussian responses. The
...
non-Gaussian responses the variance $\sigma^2$ disappears. The individual
