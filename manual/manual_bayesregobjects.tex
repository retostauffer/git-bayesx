...
Note that one, and only one, baseline term has to be specified in
the model.
\end{itemize}
...
%{\em P-spline with second order random walk penalty for
%$log(baseline)$}
...
%\item[] {\em Predictor}: $\eta =  \cdots + f_1(time)X1 + \cdots$
%\item[] {\em Description}: Defines a P-spline with a second order random walk penalty for the
%parameters of $log(baseline)$.
%\item[] {\em Example}:
%\end{itemize}
...
%\hline Time-varying effects in Cox models & X1*X2(baseline) &
%Effect of X1 varies smoothly over the range of the
%effect of X2 (time) of a Cox model. $log(\lambda_0(X1))$ is modelled by a P-spline with second order penalty. \\
\hline
\end{tabular}
...
baseline term. Note that it is compelling that there is one, and
only one baseline term specified for the vector of observed
duration times.
%Since computing the likelihood involves computing the integral
%$\int \exp(f_0(t)+\eta_i)dt$ it is no longer possible to calculate
%the likelihood analytically if the P-spline prior for $f_0(t)$
%has a degree greater than 1. Therefore the likelihood is
%approximated numerically (using the trapezoidal rule) if family=
%cox is specified.
Time-varying effects are not (yet) implemented.\\
The indicator of non-censoring $\delta_i$ has to be specified as
the dependent variable in the model statement. Data augmentation
and the specification of an offset term are not required here. To
handle left truncation and time-varying covariates a variable
#beginvar#, that records when the observation became at risk, may
be specified by adding #begin=beginvar# to
the options list.\\
...
{Bayesian P-splines.} SFB 386 discussion Paper 236, University of
Munich.
