...
listed match with the region names stored in the S-plus {\em map
...
invokes different S-plus/R functions for visualizing 2 dimensional
data. Thus, users familiar with S-plus/R may prefer to use this
functions directly to gain more flexibility.
...
This option specifies the plot type. In S-Plus there are 5 different
types available, where the default value #mode=1# corresponds to
what is called a 'Surface Plot' in S-plus. In R 3 different types
are available corresponding to a perspective plot (#mode=1#,
function #persp# in R), a contour plot (#mode=2#, function #contour#
in R), and an image plot (#mode=2#, function #image# in R) of the
data. In R #plotsurf# depends on the package #akima# for the
