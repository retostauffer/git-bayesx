...
\index{dag object!assumptions} A Gaussian dag $d$ can be
...
{\bf Representation of DAGs } \\[0.2cm]
\index{dag object!representation} To represent dags we rely on the
...
%
\index{model selection! Gaussian variables} We are not only
...
{\bf Reversible Jump Algorithm for Binary Variables}\\[0.2cm]
%
\index{model selection! Binary variables} Now we consider the
...
{\bf Reversible Jump Algorithm for Mixed Case}\\[0.2cm]
%
\index{model selection! Mixed case} For the mixed case, we assume
...
{\bf Options for initial values of algorithm}
\begin{itemize}
%\item priori\_sigma
...
{\bf Options concerning the output}
\begin{itemize}
