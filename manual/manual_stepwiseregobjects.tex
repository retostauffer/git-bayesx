...
structured additive regression (STAR)} models, see \citeasnoun{BelLan07} and \citeasnoun{FahKneLan04}. In addition to {\em
...
\end{itemize}
Inference is based on penalized likelihood in combination with fast algorithms for selection relevant covariates and model
...
given in \citeasnoun{BelLan07} and \citeasnoun{Bel07}.
...
parameterized in the first form (\ref{gammapar1_stepwise}). For the
...
\hline
{\tt criterion}  & string  & GCV       & GCV      & Generalized Cross Validation based on deviance residuals, see e.g. \citeasnoun{Woo06a} \\
