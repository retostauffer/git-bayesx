\section{Introduction}\label{mcmc:data}
...
analysed in \citeasnoun{kanlan01} and we will use the same model that has been developed there. Since our focus is on
...
\citeasnoun{fahlan01a}, \citeasnoun{fahlan01b}, \citeasnoun{lanbre04} and \citeasnoun{brelan05}. Survival models are treated in
\citeasnoun{henfah03} while count data regression is covered in \citeasnoun{fahosu03}.
...
index of the child`s mother, the district the child lives in and some further categorial covariates. Table \ref{mcmc:zambiavar}
...
\begin{tabular}{lp{12.5cm}}
 ...
 \hline
\end{tabular}
{\it\caption{Variables in the undernutrition data set. \label{mcmc:zambiavar}}}
\end{center}
\end{table}}

\section{Getting started}\label{mcmc:usage}
...
information criteria $DIC$ of the model are computed, see \citeasnoun{spibes02}. In addition, estimates for the linear
...
\epsfig{file=grafiken/zambia_mcmc_f_age1.ps,scale=0.5} {\it\caption{Effect of
...
\begin{figure}[ht]
\begin{center}
\epsfig{file=grafiken/zambia_mcmc_f_bmi3.ps,scale=0.5} {\it\caption{Effect of
...
{\it Bayesreg objects} provide some post estimation commands to get sampled parameters or to plot autocorrelation functions of
...
\epsfig{file=grafiken/zambia_mcmc_f_age3.ps,scale=0.5} {\it\caption{Results for
the nonparametric effects with hyperparameters $a=b=0.00001$ for
nonparametric and spatial effects.\label{mcmc:sensi1}}}
\end{center}
\end{figure}

Now we try two further choices for the hyperparameters, each with $a=1$ and $b$ small. We estimate models with $b=0.005$ and
$b=0.00005$:
...
\epsfig{file=grafiken/zambia_mcmc_f_age4.ps,scale=0.5} {\it\caption{Results for
...
\epsfig{file=grafiken/zambia_mcmc_f_age5.ps,scale=0.5} {\it\caption{Results for
