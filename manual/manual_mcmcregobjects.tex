...
\hline\hline
% \multicolumn{5}{l}
{1. Continuous distributions on $\dsR$}&Density&Parameters&#family#&#equationtype#\\\hline
 Normal &$p(y|\mu,\sigma^2)=\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{(y-\mu)^2}{2\sigma^2}\right)$ &$\mu\in\dsR,\sigma^2>0$ &#normal#&#sigma2#\\
 &&&#normal#&#mu#\\
 Normal &$p(y|\mu,\sigma)=\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{(y-\mu)^2}{2\sigma^2}\right)$ &$\mu\in\dsR,\sigma^2>0$ &#normal2#&#sigma#\\
  &&&#normal2#&#mu#\\
 t& $p(y|\mu,\sigma^2,\mathit{df})=\frac{\Gamma\left((\mathit{df}+1)/2\right)}{\Gamma(1/2)\Gamma(\mathit{df}/2)\sqrt{\mathit{df}\sigma^2}}\left(1+\frac{(y-\mu)^2}{\mathit{df}\sigma^2}\right)^{-\frac{\mathit{df}+1}{2}}$&$\mu\in\dsR,\mathit{df},\sigma^2>0$&#t#&#df#\\
 &&&#t#&#sigma2#\\
 &&&#t#&#mu#\\
...
Log-normal & 	$p(y|\mu,\sigma^2)=\frac{1}{\sqrt{2\pi\sigma^2} y}\exp\left(-\frac{(\log(y)-\mu)^2}{2\sigma^2}\right)$ &$\mu\in\dsR,\sigma^2>0$&#lognormal#&#sigma2# \\
&&&#lognormal#&#mu#\\
Log-normal & 	$p(y|\mu,\sigma)=\frac{1}{\sqrt{2\pi\sigma^2} y}\exp\left(-\frac{(\log(y)-\mu)^2}{2\sigma^2}\right)$ &$\mu\in\dsR,\sigma^2>0$&#lognormal2#&#sigma# \\
&&&#lognormal2#&#mu#\\
Inverse Gaussian &	$p(y|\mu,\sigma^2)=\frac{1}{\sqrt{2\pi\sigma^2} y^{3/2}}\exp\left(-\frac{(y-\mu)^2}{2 y\mu^2\sigma^2}\right)$&$\mu,\sigma^2>0$&#invgaussian# &#sigma2#\\
&&&#invgaussian#&#mu#\\
Gamma & $p(y|\mu,\sigma)=\left(\frac{\sigma}{\mu}\right)^{\sigma}\frac{y^{\sigma-1}}{\Gamma(\sigma)}\exp\left(-\frac{\sigma}{\mu}y\right)$&$\mu,\sigma>0$&#gamma#&#sigma# \\
&&&#gamma#&#mu#\\
Weibull & $	p(y|\lambda,\alpha)=\frac{\alpha y^{\alpha-1}\exp\left(-\left(y/\lambda\right)^{\alpha}\right)}{\lambda^{\alpha}}$&$\alpha,\lambda>0$&#weibull#&#alpha# \\
&&&#weibull#&#lambda#\\
Pareto & $p(y|b,c)=c b^{c}(y+c)^{-c-1}$&$b,c>0$&#pareto#&#p#\\
&&&#pareto#&#b#\\
Truncated normal &&&&\\
Generalized gamma & $p(y|\mu,\sigma,\tau)=\left(\frac{\sigma}{\mu}\right)^{\sigma\tau}\frac{\tau y^{\sigma\tau-1}}{\Gamma(\sigma)}\exp\left(-\left(\frac{\sigma}{\mu}y\right)^{\tau}\right)$&$\mu,\sigma,\tau>0$&#gengamma#&#tau#\\
&&&#gengamma#&#sigma#\\
&&&#gengamma#&#mu#\\
Dagum & $p(y|a,b,c)=\frac{a c y^{a c-1}}{b^{a c}\left(1+\left(y/b\right)^{a}\right)^{c+1}}$&$a,b,c>0$&#dagum#&#p# \\
&&&#dagum#&#b#\\
&&&#dagum#&#a#\\\hline
\multicolumn{5}{l}{3. Discrete distributions}\\\hline
Poisson & $g_1(y|\lambda)=\frac{\lambda^{y}\exp(-\lambda)}{y!}$ & $\lambda>0$&#poisson#&#lambda#\\
Negative binomial &  $g_2(y|\mu,\delta) = \frac{\Gamma(y+\delta)}{\Gamma(y+1)\Gamma(\delta)}\left(\frac{\delta}{\delta+\mu}\right)^{\delta}\left(\frac{\mu}{\delta+\mu}\right)^{y}$ &$\mu,\delta>0$&#negbin#&#delta#\\
&&&#negbin#&#mu#\\
Zero-inflated Poisson &$p(y|\lambda,\pi) = \pi\mathds{1}_{\lbrace 0\rbrace}(y)+(1-\pi)g_1$& $\pi\in(0,1)$&#zip#&#pi#\\
&&&#zip#&#lambda#\\
Zero-inflated negative binomial &$p(y|\pi,\mu,\delta) = \pi\mathds{1}_{\lbrace 0\rbrace}(y)+(1-\pi)g_2$&$\pi\in(0,1)$&#zinb#&#delta#\\
&&&#zinb#&#pi#\\
&&&#zinb#&#mu#\\
Hurdle Poisson &&&#hurdle#&#pi#\\
&&&#hurdle#&#lambda#\\
Hurdle negative binomial &&&#hurdle#&#pi#\\
&&&#hurdle#&#delta#\\
&&&#hurdle#&#mu#\\
Binomial &&&#binomial#&\\
Cloglog &&&#cloglog#&\\\hline
\multicolumn{5}{l}{4. Mixed discrete-continuous distributions}  \\\hline
...
Beta & $p(y|\mu,\sigma^2)=\frac{y^{p-1}\left(1-y\right)^{q-1}}{\B(p,q)}$&$\mu,\sigma^2\in(0,1)$&#beta#&#sigma2#\\
&$\mu=\frac{p}{p+q}$, $\sigma^2=\frac{1}{p+q+1}$&&#beta#&#mu#\\
...
Bivariate normal& &&#bivnormal#&#rho#\\
&&&#bivnormal#&#sigma#\\
&&&#bivnormal#&#sigma#\\
&&&#bivnormal#&#mu#\\
&&&#bivnormal#&#mu#\\
Bivariate normal (Fishers-$z$)& &&#bivnormal_fz#&#rho#\\
&&&#bivnormal_fz#&#sigma#\\
&&&#bivnormal_fz#&#sigma#\\
&&&#bivnormal_fz#&#mu#\\
&&&#bivnormal_fz#&#mu#\\
Bivariate t &&&#bivt#&#df#\\
 ...
 &&&#bivt#&#mu#\\
 &&&#bivt#&#mu#\\
Bivariate probit &&&#bivprobit#&#rho#\\
&&&#bivprobit#&#mu#\\
&&&#bivprobit#&#mu#\\
Bivariate logit &&&#bivlogit#&#oddsratio#\\
&&&#bivlogit#&#mu#\\
&&&#bivlogit#&#mu#\\	
