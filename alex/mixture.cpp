...
void FULLCOND_mixture::outresults(void)
  ...
  if(nosamples==false && aclag>0)
    {
    optionsp->out("  Sampled parameters may be visualized using the S-plus function 'plotsample'.");
    optionsp->out("  Autocorrelation functions may be visualized using the S-Plus function 'plotautocor'.");
    optionsp->out("\n");
    optionsp->out("\n");
    }
  if(nosamples==false && aclag==0)
