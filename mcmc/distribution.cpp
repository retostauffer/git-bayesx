...
void DISTRIBUTION::create_bootstrap_weights(void)   // wie bei MSEP und CV???
  ...
  if(response_ori.rows()==1)
    ...
    linearpred_ori = linearpred;     // speichert Pr�diktor vom Original-Datensatz
    }
...
void DISTRIBUTION::create_weight(datamatrix & w, const double & p1, const bool & fertig, const bool & CV)
  ...
  if(fertig == true)        // erstellt neue Gewichte f�r MSEP / AUC
    {
  ...
  if(CV == true)
    ...
    if(weight.max(0) != 1 || weight.max(0) != 1)  // falls Gewichte im Befehl angegeben wurden, werden diese ber�cksuchtigt
      {
      double * workw = weight.getV();
...
/*double DISTRIBUTION_gaussian::compute_logmsep(void)
  ...
  for (i=0;i<nrobs;i++,worklin++,workresp++,workweight++,work2++)
    ...
    if (*workweight == 0)
      ...
      //sum += *workweight*help*help;     // u.U. �ndern, so dass zwei Variablen, eine mit
...
double DISTRIBUTION_binomial::compute_auc(void)
  {
  datamatrix linpred_null_hilf = datamatrix(nrobs,1,0);       // Erzeugen einer Matrix die Eintr�ge f�r Beob.
                                                              // mit weight=0 enth�lt und darunter Nuller
  datamatrix response_null_hilf = datamatrix(nrobs,1,0);      // Erzeugen einer Matrix die Eintr�ge f�r Beob.
                                                              // mit weight=0 enth�lt und darunter Nuller
  ...
  datamatrix linpred_null = datamatrix(nrnull,1,0);  // K�rzen des "Null"-Pr�diktors, d.h. Weglassen der Nullen
  datamatrix response_null = datamatrix(nrnull,1,0); // K�rzen des "Null"-Response, d.h. Weglassen der Nullen
  ...
  statmatrix<int> index_gesamt(linpred_null.rows(),1);      // Sortieren des linearen Pr�diktors
  ...
  linpred_null.rank(rang_gepoolt,index_gesamt,0,linpred_null.rows()-1,0);  // R�nge berechnen
