...
class __EXPORT_TYPE DISTRIBUTION
  ...
  bool iwlsweights_notchanged_df;    // f�r "stepwise": gibt an, ob gegen�ber dem letzten Mal "df" berechnen die Gewichte ver�ndert wurden
  double gcvfactor;
  int seed;
...
//    }
  ...
  // FUNCTION: tilde_y
  // TASK: computes tildey = m + (y-mu)g'(mu)      ( f�r IWLS )
...
//
// Implementiert sind drei M�glichkeiten:
//
//
// 1. FIXER SKALENPARAMETER (Konstruktor 0)
//
// Phi wird im Konstruktor 0 einem bekannten, �ber alle Iterationen konstant
// bleibenden Wert gleichgesetzt.
//
//
// 2. KONSISTENTE SCH�TZUNG DES SKALENPARAMETERS (Konstruktor 1)
//
// Phi wird in jeder Iteration der Simulation mit phi_hut
// (vgl. Fahrmeir/Tutz S.44) konsistent gesch�tzt.
//
// Zur Verbesserung und Beschleunigung von Konvergenz und Mixing des
// Gesamtmodells kann phi zu Beginn f�r eine bestimmte Anzahl von Iterationen
// (g�nstig ca. 500) konstant gehalten werden, siehe Konstruktor 1.
//
//
...
// Die feste Varianz mu� als Tuning Parameter im Konstruktor 2 geeignet
// gew�hlt werden. Dabei gilt: F�r gro�e Varianzen ist die Akzeptanz des
...
// Iteration konsistent gesch�tzt. (Dazu ist ebenfalls wieder ein Konstanthalten
// in den allerersten Iterationen m�glich.)
//------------------------------------------------------------------------------
